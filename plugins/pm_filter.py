...
from info import ADMINS, CLONE_MODE, AUTH_CHANNEL, REQST_CHANNEL, AUTH_USERS, CUSTOM_FILE_CAPTION, AUTH_GROUPS, P_TTI_SHOW_OFF, IMDB, SINGLE_BUTTON, SPELL_CHECK_REPLY, IMDB_TEMPLATE, LOG_CHANNEL, FILE_STORE_CHANNEL, MAX_BTN, UN_CHANNEL
from utils import get_poster, temp, get_settings, save_group_settings, is_subscribed
...
async def pm_text(bot, message):
    ...
    # Check force subscribe for PM searches
    if not await is_subscribed(bot, message):
        from plugins.commands import check_force_subscribe
        not_subscribed_channels = await check_force_subscribe(bot, message)
        if not_subscribed_channels:
            try:
                btn = []
                for channel_id in not_subscribed_channels:
                    try:
                        from info import REQUEST_TO_JOIN_MODE
                        if REQUEST_TO_JOIN_MODE == True:
                            invite_link = await bot.create_chat_invite_link(chat_id=int(channel_id), creates_join_request=True)
                        else:
    ...
    if await db.get_pm_search_status(message.from_user.id):
...
async def send_fall_cb_handler(client: Client, query: CallbackQuery):
...
async def send_fsall_cb_handler(client: Client, query: CallbackQuery):
    temp_var, ident, key, offset = query.data.split("#")
...
async def kill_files_dq_cb_handler(client: Client, query: CallbackQuery):
...
async def open_settings_grp_cb_handler(client: Client, query: CallbackQuery):
...
async def open_settings_pm_cb_handler(client: Client, query: CallbackQuery):
    ident, grp_id = query.data.split("#")
    userid = query.from_user.id if query.from_user else None
    st = await client.get_chat_member(grp_id, userid)
    if (
...
async def show_option_cb_handler(client: Client, query: CallbackQuery):
...
async def unavailable_cb_handler(client: Client, query: CallbackQuery):
    ident, from_user = query.data.split("#")
    btn = [[
...
async def uploaded_cb_handler(client: Client, query: CallbackQuery):
    ident, from_user = query.data.split("#")
    btn = [[
...
async def already_available_cb_handler(client: Client, query: CallbackQuery):
    ident, from_user = query.data.split("#")
    btn = [[
...
async def alalert_cb_handler(client: Client, query: CallbackQuery):
    ident, from_user = query.data.split("#")
...
async def upalert_cb_handler(client: Client, query: CallbackQuery):
    ident, from_user = query.data.split("#")
    if int(query.from_user.id) == int(from_user):
        user = await client.get_users(from_user)
...
async def unalert_cb_handler(client: Client, query: CallbackQuery):
    ident, from_user = query.data.split("#")
    if int(query.from_user.id) == int(from_user):
        user = await client.get_users(from_user)
...
async def generate_stream_link_cb_handler(client: Client, query: CallbackQuery):
...
async def reqinfo_cb_handler(client: Client, query: CallbackQuery):
    await query.answer(text=script.REQINFO, show_alert=True)

@Client.on_callback_query(filters.regex(r"^select"))
async def select_cb_handler(client: Client, query: CallbackQuery):
    await query.answer(text=script.SELECT, show_alert=True)

@Client.on_callback_query(filters.regex(r"^sinfo"))
async def sinfo_cb_handler(client: Client, query: CallbackQuery):
    await query.answer(text=script.SINFO, show_alert=True)

@Client.on_callback_query(filters.regex(r"^start"))
async def start_cb_handler(client: Client, query: CallbackQuery):
...
async def clone_cb_handler(client: Client, query: CallbackQuery):
    buttons = [[
...
async def filters_cb_handler(client: Client, query: CallbackQuery):
    buttons = [[
...
async def global_filters_cb_handler(client: Client, query: CallbackQuery):
    buttons = [[
...
async def help_cb_handler(client: Client, query: CallbackQuery):
    buttons = [[
...
async def about_cb_handler(client: Client, query: CallbackQuery):
    buttons = [[
...
async def subscription_cb_handler(client: Client, query: CallbackQuery):
    buttons = [[
...
async def manuelfilter_cb_handler(client: Client, query: CallbackQuery):
    buttons = [[
        InlineKeyboardButton('⟸ Bᴀᴄᴋ', callback_data='filters'),
...
async def button_cb_handler(client: Client, query: CallbackQuery):
    buttons = [[
...
async def coct_cb_handler(client: Client, query: CallbackQuery):
    buttons = [[
        InlineKeyboardButton('⟸ Bᴀᴄᴋ', callback_data='help')
    ]]
    await client.edit_message_media(
...
async def admin_cb_handler(client: Client, query: CallbackQuery):
    buttons = [[
        InlineKeyboardButton('⟸ Bᴀᴄᴋ', callback_data='help'),
...
async def store_file_cb_handler(client: Client, query: CallbackQuery):
    buttons = [[
        InlineKeyboardButton('⟸ Bᴀᴄᴋ', callback_data='help')
    ]]
    await client.edit_message_media(
...
async def r_txt_cb_handler(client: Client, query: CallbackQuery):
    buttons = [[
        InlineKeyboardButton('⟸ Bᴀᴄᴋ', callback_data='help')
    ]]
    await client.edit_message_media(
...
async def s_txt_cb_handler(client: Client, query: CallbackQuery):
    buttons = [[
        InlineKeyboardButton('⟸ Bᴀᴄᴋ', callback_data='help')
    ]]
    await client.edit_message_media(
...
async def extra_cb_handler(client: Client, query: CallbackQuery):
    buttons = [[
...
async def stats_cb_handler(client: Client, query: CallbackQuery):
    buttons = [[
        InlineKeyboardButton('⟸ Bᴀᴄᴋ', callback_data='help'),
...
async def rfrsh_cb_handler(client: Client, query: CallbackQuery):
...
async def shortlink_info_cb_handler(client: Client, query: CallbackQuery):
    btn = [[
...
async def tele_cb_handler(client: Client, query: CallbackQuery):
    btn = [[
        InlineKeyboardButton("⟸ Bᴀᴄᴋ", callback_data="help"),
        InlineKeyboardButton("Cᴏɴᴛᴀᴄᴛ", url="telegram.me/KingVj01")
    ]]
...
async def ytdl_cb_handler(client: Client, query: CallbackQuery):
    buttons = [[
...
async def share_cb_handler(client: Client, query: CallbackQuery):
    btn = [[
        InlineKeyboardButton("⟸ Bᴀᴄᴋ", callback_data="help"),
        InlineKeyboardButton("Cᴏɴᴛᴀᴄᴛ", url="telegram.me/KingVj01")
    ]]
...
async def song_cb_handler(client: Client, query: CallbackQuery):
    btn = [[
        InlineKeyboardButton("⟸ Bᴀᴄᴋ", callback_data="help"),
        InlineKeyboardButton("Cᴏɴᴛᴀᴄᴛ", url="telegram.me/KingVj01")
    ]]
...
async def json_cb_handler(client: Client, query: CallbackQuery):
    buttons = [[
        InlineKeyboardButton('⇍ ʙᴀᴄᴋ ⇏', callback_data='help')
    ]]
    reply_markup = InlineKeyboardMarkup(buttons)
...
async def sticker_cb_handler(client: Client, query: CallbackQuery):
    btn = [[
        InlineKeyboardButton("⟸ Bᴀᴄᴋ", callback_data="help"),
        InlineKeyboardButton("Cᴏɴᴛᴀᴄᴛ", url="telegram.me/KingVj01")
    ]]
...
async def tamil_info_cb_handler(client: Client, query: CallbackQuery):
    btn = [[
        InlineKeyboardButton("⟸ Bᴀᴄᴋ", callback_data="start"),
        InlineKeyboardButton("Cᴏɴᴛᴀᴄᴛ", url="telegram.me/KingVj01")
    ]]
...
async def english_info_cb_handler(client: Client, query: CallbackQuery):
    btn = [[
        InlineKeyboardButton("⟸ Bᴀᴄᴋ", callback_data="start"),
        InlineKeyboardButton("Cᴏɴᴛᴀᴄᴛ", url="telegram.me/KingVj01")
    ]]
...
async def hindi_info_cb_handler(client: Client, query: CallbackQuery):
    btn = [[
        InlineKeyboardButton("⟸ Bᴀᴄᴋ", callback_data="start"),
        InlineKeyboardButton("Cᴏɴᴛᴀᴄᴛ", url="telegram.me/KingVj01")
    ]]
...
async def telugu_info_cb_handler(client: Client, query: CallbackQuery):
    btn = [[
        InlineKeyboardButton("⟸ Bᴀᴄᴋ", callback_data="start"),
        InlineKeyboardButton("Cᴏɴᴛᴀᴄᴛ", url="telegram.me/KingVj01")
    ]]
...
async def malayalam_info_cb_handler(client: Client, query: CallbackQuery):
    btn = [[
        InlineKeyboardButton("⟸ Bᴀᴄᴋ", callback_data="start"),
        InlineKeyboardButton("Cᴏɴᴛᴀᴄᴛ", url="telegram.me/KingVj01")
    ]]
...
async def urdu_info_cb_handler(client: Client, query: CallbackQuery):
    btn = [[
        InlineKeyboardButton("⟸ Bᴀᴄᴋ", callback_data="start"),
        InlineKeyboardButton("Cᴏɴᴛᴀᴄᴛ", url="telegram.me/KingVj01")
    ]]
...
async def bangladesh_info_cb_handler(client: Client, query: CallbackQuery):
    btn = [[
        InlineKeyboardButton("⟸ Bᴀᴄᴋ", callback_data="start"),
        InlineKeyboardButton("Cᴏɴᴛᴀᴄᴛ", url="telegram.me/KingVj01")
    ]]
...
async def kannada_info_cb_handler(client: Client, query: CallbackQuery):
    btn = [[
        InlineKeyboardButton("⟸ Bᴀᴄᴋ", callback_data="start"),
        InlineKeyboardButton("Cᴏɴᴛᴀᴄᴛ", url="telegram.me/KingVj01")
    ]]
...
async def gujarati_info_cb_handler(client: Client, query: CallbackQuery):
    btn = [[
        InlineKeyboardButton("⟸ Bᴀᴄᴋ", callback_data="start"),
        InlineKeyboardButton("Cᴏɴᴛᴀᴄᴛ", url="telegram.me/KingVj01")
    ]]
...
async def setgs_cb_handler(client: Client, query: CallbackQuery):
...
async def auto_filter(client, name, msg, reply_msg, ai_search, spoll=False):
    ...
    if imdb:
        cap = TEMPLATE.format(
            ...
            kind=imdb['kind'],            imdb_id=imdb["imdb_id"],
            cast=imdb["cast"],
...
async def filter_qualities_cb_handler(client: Client, query: CallbackQuery):
    ...
    if settings["button"]:
        ...
        btn.insert(0, [
            InlineKeyboardButton("𝐒𝐞𝐧𝐝 𝐀𝐥ｌ", callback_data=f"sendfiles#{key}"),
            InlineKeyboardButton("ʟᴀɴɢᴜᴀɢᴇs", callback_data=f"languages#{key}"),
            InlineKeyboardButton("ʏᴇᴀʀs", callback_data=f"years#{key}")
        ])
...
async def setgs_cb_handler(client: Client, query: CallbackQuery):
    ...
    if settings is not None:
        buttons = [
            ...
            [
                InlineKeyboardButton('Sᴘᴇʟʟ CʜᴇᴄK',
                                     callback_data=f'setgs#spell_check#{settings["spell_check"]}#{str(grp_id)}'),
                InlineKeyboardButton('✔ Oɴ' if settings["spell_check"] else '✘ Oғғ',
                                     callback_data=f'setgs#spell_check#{settings["spell_check"]}#{str(grp_id)}')
            ],
